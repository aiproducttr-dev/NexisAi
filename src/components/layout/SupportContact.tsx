...
function SupportCard({
  ...
  return (
    <a
      ...
      className={`group flex items-center rounded-2xl border border-white/10 bg-gradient-to-br from-[#0a0a12]/95 to-cyan-500/[0.08] shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_8px_32px_rgba(0,0,0,0.35)] backdrop-blur-md transition duration-300 hover:-translate-y-0.5 hover:border-cyan-500/35 hover:shadow-[0_8px_32px_rgba(6,182,212,0.16),inset_0_1px_0_rgba(255,255,255,0.08)] ${
        compact
          ? "w-full max-w-md gap-2.5 px-3 py-2.5 sm:max-w-[22rem] sm:px-4 sm:py-3"
    ...
    >
      <span
        ...
        aria-hidden
      >
        <Mail className={`text-cyan-400 ${compact ? "h-4 w-4" : "h-5 w-5"}`} />
      </span>
      ...
      <span className="min-w-0 flex-1 text-left">
        ...
        <span
          className={`mt-0.5 block truncate font-semibold text-white transition group-hover:text-cyan-300 ${
            compact ? "text-xs sm:text-sm" : "mt-1 text-sm sm:text-base"
          }`}
        >
          {SUPPORT_EMAIL}
        </span>
        <span
          className={`block leading-relaxed text-[#64748b] ${
            compact ? "mt-0.5 text-[10px] sm:text-xs" : "mt-0.5 text-xs"
          }`}
        >
...
export default function SupportContact({
  ...
  if (variant === "topRight") {
    return (
      <>
        {/* Mobile: in document flow, never overlays the logo */}
        <div className={`flex justify-center md:hidden ${className}`}>
          <SupportCard compact className="w-full max-w-md" />
        </div>

        {/* Desktop: fixed top-right */}
        <div
          className={`pointer-events-none fixed right-5 z-50 hidden w-[min(100vw-2.5rem,22rem)] md:block ${
            belowNav ? "top-[5.25rem]" : "top-5"
          }`}
        >
